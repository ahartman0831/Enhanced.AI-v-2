...
export async function GET() {
  try {
    ...
    if (error) {
      console.error('Recovery reports fetch error:', error)
      return NextResponse.json({ error: 'Failed to fetch recovery analyses' }, { status: 500 })
    }

    const reports = (data || []).filter((row) => {
      const json = row.stack_json as Record<string, unknown>
      return json?.analysisType === 'recovery-timeline' && json?.recoveryAnalysis != null
    })

    return NextResponse.json({ reports })
  } catch (err) {
    console.error('Recovery reports API error:', err)
